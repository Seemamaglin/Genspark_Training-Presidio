...
public class AdminController : ControllerBase {
    ...
    {
        var op = await _context.BusOperators
            .Include(o => o.User)
            .Include(o => o.Buses).ThenInclude(b => b.Bookings).ThenInclude(bo => bo.Payment)
            .FirstOrDefaultAsync(o => o.Id == operatorId);
        if (op == null) return NotFound();
        var revenue = op.Buses?.SelectMany(b => b.Bookings ?? new List<Booking>()).Where(bo => bo.Payment != null && bo.Payment.Status == PaymentStatus.Success).Sum(bo => bo.Payment?.Amount ?? 0) ?? 0;
        return Ok(new { OpId = operatorId, OpName = op.User != null ? op.User.Name : string.Empty, Revenue = revenue });
    }

    [HttpPost("buses/{busId}/cancel")]
    public async Task<IActionResult> CancelBus(Guid busId, [FromBody] CancelBusRequest request)
    {
        var bus = await _context.Buses.Include(b => b.Bookings).ThenInclude(bo => bo.User).FirstOrDefaultAsync(b => b.Id == busId);
        if (bus == null) return NotFound();
        bus.IsActive = false;
        var bookings = bus.Bookings?.Where(b => b.Status == BookingStatus.Confirmed).ToList() ?? new List<Booking>();
